...
var _ = suite("compute/image/list-distribution", func(t *testing.T, when spec.G, it spec.S) {
	var (
	...
	it.Before(func() {
		...
		server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			case "/v2/images":
				...
				if req.URL.Query().Get("type") != "distribution" {
	...
	when("passing no flags", func() {
		it("lists all distribution images", func() {
